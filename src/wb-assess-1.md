...
```
`console.log` prints whatever you put into it into the console.
`return` ends a function and returns whatever is specified to the function caller.
```

**_What's the difference between defining and calling a function?_**

```
You must define a function to be able to call it, but you don't have to call a function to define it.
Defining a function is creating the function, and calling it is running the function.
```
...
```
A string is an unbroken line of text, numbers, special characters, or whitespace.
```

**_Number_**

```
A numerical value, whether it be positive, negative, or a decimal.
```

**_Boolean_**

```
True or false. There are only these two options with Boolean data types.
```

**_Array_**

```
An array is a container of data, it can contain multiple different data types.
```
